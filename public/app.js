...
/* =========================================================
   CALLIBRI RISK REDESIGN BRIDGE
   Связывает новый левый бар с уже рабочей логикой app.js.
   ========================================================= */

(function callibriRiskRedesignBridge() {
  ...
  function makeAvatarOpenProfile() {
    const avatar = document.getElementById("profileAvatarBtn");

    if (!avatar || avatar.dataset.avatarSettingsBound === "1") return;

    avatar.dataset.avatarSettingsBound = "1";
  ...
  function patchSafe(functionName) {
    ...
    if (typeof original !== "function") return;
    if (original.__riskBridgePatched) return;

    const wrapped = function () {
      const result = original.apply(this, arguments);

      setTimeout(runBridge, 0);
      setTimeout(runBridge, 160);

      return result;
    };

    wrapped.__riskBridgePatched = true;
    window[functionName] = wrapped;
  }
